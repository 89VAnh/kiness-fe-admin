...
export default function ObesityStoryModal({
  id,
  isCreate = true,
}: Props): JSX.Element {
  ...
  const createObesityStory = useCreateObesityStory({
    config: {
      onSuccess: (data) => {
        if (data.success) {
          message.success(t("messages.update_success"));
          handleCancel();
          queryClient.invalidateQueries([CACHE_OBESITY_STORY.SEARCH]);
  ...
  const updateObesityStory = useUpdateObesityStory({
    config: {
      onSuccess: (data) => {
        if (data.success) {
          message.success(t("messages.update_success"));
    ...
    form
      ...
      .then(async (values) => {
        ...
        if (isCreate) {
          ...
          createObesityStory.mutate(dataPost);
        } else {
  ...
  return (
    <>
      ...
      <Modal
        title={
          isCreate
            ? t("obesity_story.title_create")
            : t("obesity_story.title_update")
        }
        style={{ top: 58, padding: 0, minWidth: 1000 }}
        open={isOpen}
        onCancel={handleCancel}
        onOk={handleSubmit}
        confirmLoading={updateObesityStory.isLoading}
        maskClosable={false}
      >
        <div
          style={{
