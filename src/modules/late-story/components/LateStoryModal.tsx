...
export default function LateStoryModal({
  id,
  isCreate = true,
}: Props): JSX.Element {
  ...
  const createLateStory = useCreateLateStory({
    config: {
      onSuccess: (data) => {
        if (data.success) {
  ...
  const updateLateStory = useUpdateLateStory({
    config: {
      onSuccess: (data) => {
        if (data.success) {
          message.success(t("messages.update_success"));
    ...
    form
      ...
      .then(async (values) => {
        ...
        if (isCreate) {
          ...
          updateLateStory.mutate(dataPost);
        }
        setDataEditor("");
      })
      .catch((err) => {
  ...
  return (
    <>
      ...
      <Modal
        ...
        onOk={handleSubmit}
        confirmLoading={updateLateStory.isLoading}
        maskClosable={false}
      >
        <div
          style={{
